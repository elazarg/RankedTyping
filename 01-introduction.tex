...
Alternatively, grades can be read as a form of \emph{quantitative taint}: each coercion site introduces uncertainty that propagates forward via $\max$, so the grade of a value measures the depth of the speculation chain that produced it.
...
This is confirmed by the implementation: the analyzer is parameterized over a single coercion function, and substituting the grade successor $n\mapsto\infty$ (in place of $n\mapsto n{+}1$) reproduces gradual typing exactly.
...
\begin{itemize}
  ...
  \item A concrete demonstration that standard gradual typing is a special case of graded typing: the abstract interpreter, parameterized over the coercion function, reproduces gradual typing by changing only the grade successor ($n\mapsto\infty$).
\end{itemize}
