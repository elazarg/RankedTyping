...
The key insight is a gradeed algebra of types where base types and $\Any$ carry a nonnegative grade $i$; grades increase \emph{only} when a use forces a base after imprecision (the subtyping step $\Any^{i}\!\le\!X^{i+1}$), while heterogeneous joins are conservative ($X^{r}\sqcup Y^{r}=\Any^{r}$ after promotion).
