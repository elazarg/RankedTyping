...
A cost-based interpretation selection for overloaded operators yields principal (most precise) abstract types.
...
Standard gradual typing is recovered by collapsing grades ($X^{i}\mapsto X$, $\Any^{i}\mapsto ?$) and reintroducing consistency and casts; we demonstrate this concretely by parameterizing the abstract interpreter over a single coercion function, with the gradual instantiation differing only in the grade successor ($n \mapsto \infty$ instead of $n \mapsto n{+}1$).
