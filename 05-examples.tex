...
\paragraph{Example 10 (precise arithmetic pipeline).}
\begin{quote}\ttfamily
x := 1;\quad y := x + 2;\quad z := y + 3
\end{quote}
Both operands of each $+$ are $\Int^{0}$; cost-based selection picks the $\Int$ interpretation (cost $0$) over $\String$ (cost $2$). Grade stays $0$ throughout:
\[
\Gamma_1 \;=\; \Gamma_0[\,x \mapsto \Int^{0},\;
                       y \mapsto \Int^{0},\;
                       z \mapsto \Int^{0}\,].
\]
Under a na\"ive ``join all interpretations'' rule, each $+$ would join $\Int$ and $\String$ results, producing $z:\Any^{1}$---a spurious loss of precision.

\paragraph{Example 11 (grade sticks but does not grow).}
\begin{quote}\ttfamily
if b then x := 1 else x := "hi";\\
y := x + 1;\quad z := y + 1
\end{quote}
After the branch: $x:\Any^{0}$.
For $y := x + 1$: coercing $\Any^{0}$ to $\Int$ costs $1$, to $\String$ also costs $1$; the literal $1$ costs $0$ for $\Int$ and $1$ for $\String$. Total: $\Int$ costs $1$, $\String$ costs $2$. The $\Int$ interpretation wins: $y:\Int^{1}$.
For $z := y + 1$: both operands are already $\Int$ (costs $0$); grade does not grow: $z:\Int^{1}$.
\[
\Gamma_1 \;=\; \Gamma_0[\,x \mapsto \Any^{0},\;
                       y \mapsto \Int^{1},\;
                       z \mapsto \Int^{1}\,].
\]
Once committed to a base (even at grade $>0$), further same-base operations are free.

\paragraph{Example 12 (graded versus gradual comparison).}
\begin{quote}\ttfamily
a := 1;\quad b := a + 2;\\
if c then x := 1 else x := "hi";\\
d := x + 1;\\
e := d + x
\end{quote}
Under graded typing:
$a:\Int^{0}$, $b:\Int^{0}$ (precise pipeline).
After the branch: $x:\Any^{0}$.
$d:\Int^{1}$ ($\Int$ preferred, one coercion from $x$).
$e:\Int^{1}$ ($d$ and $x$ both coerce to $\Int$ at grade $\le 1$; $\max(1,1)=1$).

Under gradual typing (collapsing grades to $\{0,\infty\}$; see \S\ref{sec:extensions}):
$a:\Int^{0}$, $b:\Int^{0}$ (same), but $d:\Int^{\infty}$ and $e:\Int^{\infty}$---the distinction between ``one speculation'' and ``two speculations'' is lost.
\[
\begin{array}{r@{\;\mapsto\;}l@{\qquad}r@{\;\mapsto\;}l}
\multicolumn{2}{c}{\textbf{Graded}} & \multicolumn{2}{c}{\textbf{Gradual}} \\[2pt]
a & \Int^{0} & a & \Int^{0} \\
b & \Int^{0} & b & \Int^{0} \\
d & \Int^{1} & d & \Int^{\infty} \\
e & \Int^{1} & e & \Int^{\infty}
...
\paragraph{Example 13 (mixing bases through a later join).}
\begin{quote}\ttfamily
...
\paragraph{Example 14 (guard typing is orthogonal).}
\begin{quote}\ttfamily
...
\paragraph{Example 15 (absorption with a precise base).}
\begin{quote}\ttfamily
if b then x := 1 else x := "a";\\
y := 0;\\
...
\paragraph{Example 16 (disjoint effects on different variables).}
\begin{quote}\ttfamily
...
\paragraph{Example 17 (branch-local refinement, global imprecision).}
\begin{quote}\ttfamily
if b then x := 1 else x := "a";\\
...
\paragraph{Example 18 (loop head as a choke point).}
\begin{quote}\ttfamily
x := 0;\\
...
\paragraph{Example 19 (loop with branch-induced escalation).}
\begin{quote}\ttfamily
x := 0;\\
...
\paragraph{Example 20 (boolean loop).}
\begin{quote}\ttfamily
x := true;\\
while c do x := $\neg$ x
\end{quote}
...
\paragraph{Example 21 (propagating imprecision through copy and use).}
\begin{quote}\ttfamily
...
\paragraph{Example 22 (multiple variables through one choke point).}
\begin{quote}\ttfamily
if b then \{ x := 1; y := "a" \} else \{ x := "b"; y := 2 \}
\end{quote}
Pointwise join yields $x:\Any^{0}$ and $y:\Any^{0}$.

\paragraph{Example 23 (no-op branches, no escalation).}
\begin{quote}\ttfamily
